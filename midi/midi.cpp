...
void Midi::note_on(uint8_t channel, uint8_t note, uint8_t velocity) {
...
void Midi::note_off(uint8_t channel, uint8_t note, uint8_t velocity) {
    m_pop_note(note);
    m_last_note = note_stack[0];
    note_playing = (note_stack[0] != -1);
}

void Midi::pitch_bend(uint8_t channel, uint16_t bend) {
    this->bend = bend;
...
void Midi::m_push_note(uint8_t note) {
...
void Midi::m_pop_note(uint8_t note) {
    bool shift = false;
...
int Midi::m_find_note(uint8_t note) {
